...
const CS_GOOGLE_FONTS = [
    ...
    { name: 'Michroma',         category: 'Sci-Fi'       },
    { name: 'Share Tech Mono',  category: 'Sci-Fi'       },
    { name: 'Quantico',         category: 'Sci-Fi'       },
    { name: 'Iceland',          category: 'Sci-Fi'       },
    // Fantasy
    { name: 'Cinzel',           category: 'Fantasy'      },
    { name: 'Cinzel Decorative',category: 'Fantasy'      },
    { name: 'Uncial Antiqua',   category: 'Fantasy'      },
    { name: 'IM Fell English',  category: 'Fantasy'      },
    { name: 'MedievalSharp',    category: 'Fantasy'      },
    { name: 'Almendra',         category: 'Fantasy'      },
    { name: 'Spectral',         category: 'Fantasy'      },
    { name: 'Pirata One',       category: 'Fantasy'      },
    // Gaming
    { name: 'Press Start 2P',   category: 'Gaming'       },
    { name: 'VT323',            category: 'Gaming'       },
    { name: 'Boogaloo',         category: 'Gaming'       },
    { name: 'Russo One',        category: 'Gaming'       },
    { name: 'Wallpoet',         category: 'Gaming'       },
    // Sans-Serif
...
// Walk up the Fabric group chain to the top-level canvas object.
function _csRootObj(obj) {
    if (!obj) return null;
    let o = obj;
...
async function initStudio() {
    ...
    _csRenderStatPosTiles();
    _csLoadStudioLayouts();
...
window.csAddTraitZone = function() {
    ...
    // Default to genesis + normal trait visible
    const genesisChk = document.getElementById('cs-tz-genesis');
    if (genesisChk) genesisChk.checked = true;
...
function _csTzMinSize() {
    ...
    let rowH;
    if (showName) {
        rowH = showDesc ? nameSize + descSize + 12 : nameSize + 12;
    } else {
        rowH = iconSize + 10;
    }

    const rows  = isGenesis ? 2 : 1;
    const minH  = rows * rowH + 24;  // 12px top + 12px bottom padding
    const minW  = Math.max(120, iconSize * 3);
    return { minW, minH };
}

function _csEnforceTzMinSize(obj) {
    if (!obj?.data?.isTraitZone) return;
    const { minW, minH } = _csTzMinSize();
    const renderedW = (obj.width  || 0) * (obj.scaleX || 1);
    const renderedH = (obj.height || 0) * (obj.scaleY || 1);
    if (renderedW < minW) obj.set('scaleX', minW / (obj.width || 1));
    if (renderedH < minH) obj.set('scaleY', minH / (obj.height || 1));
    obj.setCoords();
}

function _csClampToArtboard(obj) {
    if (!obj) return;
    const shouldClamp = obj.data?.layerType === 'name' || obj.data?.isTraitZone === true;
    if (!shouldClamp) return;
    // Re-fetch bounding rect after any snapping has already run
    const br = obj.getBoundingRect(true);
    let newLeft = obj.left;
    let newTop  = obj.top;
    if (br.left < 0)                      newLeft = obj.left - br.left;
    if (br.top  < 0)                      newTop  = obj.top  - br.top;
    if (br.left + br.width  > CS_W)       newLeft = obj.left - (br.left + br.width  - CS_W);
    if (br.top  + br.height > CS_H)       newTop  = obj.top  - (br.top  + br.height - CS_H);
...
function _csBindEventsExtra() {
    ...
    _csCanvas.on('object:moving', e => {
        ...
        // Hard clamp AFTER snapping — name and trait zone cannot leave the artboard
        _csClampToArtboard(obj);
    });

    _csCanvas.on('mouse:up', () => _csClearGuides());
    _csCanvas.on('object:scaling', e => { _csEnforceTzMinSize(e.target); });
    _csCanvas.on('object:modified', e => { _csClampToArtboard(e.target); _csEnforceTzMinSize(e.target); });
...
function csTool(tool) {
    ...
    if (tool === 'eraser') {
        ...
        _csHandlers._canvasDown = opt => {
            ...
            const offW = el.naturalWidth || obj.width || el.width || 1;
            const offH = el.naturalHeight || obj.height || el.height || 1;
            _eraseOffscreen = document.createElement('canvas');
            _eraseOffscreen.width = offW;
            _eraseOffscreen.height = offH;
            try {
                _eraseOffscreen.getContext('2d').drawImage(el, 0, 0, offW, offH);
            } catch {
                // Cross-origin image — fetch as blob first then retry
                isErasing = false;
                _eraseOffscreen = null;
                _eraseObj = null;
                fetch(el.src, { mode: 'cors' })
                    .then(r => r.blob())
                    .then(blob => {
                    ...
                    .then(safeImg => {
                        fabric.Image.fromURL(safeImg.src, reloaded => {
                            reloaded.set({
                            ...
                            reloaded.data = { ...obj.data };
                            const idx2 = c.getObjects().indexOf(obj);
                            c.remove(obj);
                            if (idx2 >= 0) c.insertAt(reloaded, idx2); else c.add(reloaded);
                            c.setActiveObject(reloaded);
                            c.renderAll();
                            _csPushHistory();
                            _csRenderLayerList();
                            showToast('Image reloaded for erasing — try again', 'info');
                        }, { crossOrigin: 'anonymous' });
                    })
                    .catch(() => showToast('Cannot erase: image is cross-origin protected', 'error'));
                return;
            }
            _eraseAt(opt.e);
        };
        ...
        _csHandlers._canvasUp = async () => {
            ...
            _eraseOffscreen = null;
            _eraseObj = null;
            const blob = await new Promise(res => { try { off.toBlob(res, 'image/png'); } catch { res(null); } });
            if (!blob) { showToast('Could not finalise erase', 'error'); return; }
...
function _csRenderContextProps(obj) {
    ...
    if (type === 'text' || obj.type === 'i-text' || obj.type === 'text' || obj.type === 'textbox') {
        ...
        const fontOptItems = CS_GOOGLE_FONTS.map(f =>
            `<div class="cs-dd-opt cs-font-ctx-opt" style="font-size:12px;padding:6px 10px;font-family:'${escapeHTML(f.name)}',sans-serif;" data-v="${escapeHTML(f.name)}">${escapeHTML(f.name)}</div>`
        ).join('');
        ...
        html = `
            <div class="cs-dd" id="cs-font-ctx-dd" style="min-width:140px;">
                <button type="button" class="cs-dd-trigger" style="font-size:11px;padding:5px 10px;font-family:'${escapeHTML(curFont)}',sans-serif;">
                    <span class="cs-dd-label">${escapeHTML(curFont)}</span>
                    <i class="bx bx-chevron-down cs-dd-chevron"></i>
                </button>
                <div class="cs-dd-menu" hidden style="max-height:280px;min-width:160px;">
                    <div style="padding:4px 4px 6px;">
                        <input type="text" class="cs-font-ctx-search" placeholder="Search fonts…"
                            style="width:100%;background:oklch(0% 0 0 / 0.3);border:1px solid rgba(var(--ar),0.2);border-radius:6px;color:var(--text);padding:5px 8px;font-size:10px;font-family:inherit;outline:none;"
                            oninput="_csFontCtxFilter(this)">
                    </div>
                    <div class="cs-font-ctx-list">${fontOptItems}</div>
                </div>
            </div>
...
/* ─── Font context-bar custom dropdown ──────────────────────────────────── */
window._csFontCtxFilter = function(inp) {
    const q = inp.value.toLowerCase();
    inp.closest('.cs-dd-menu').querySelector('.cs-font-ctx-list')
        .querySelectorAll('.cs-font-ctx-opt')
        .forEach(opt => { opt.hidden = !!q && !opt.dataset.v.toLowerCase().includes(q); });
};

function _csInitFontCtxDD() {
    const wrap = document.getElementById('cs-font-ctx-dd');
    ...
    trigger.onclick = e => {
        ...
        if (menu.hidden) {
            ...
            const srch = menu.querySelector('.cs-font-ctx-search');
            if (srch) { srch.value = ''; _csFontCtxFilter(srch); setTimeout(() => srch.focus(), 50); }
            const handler = e2 => { if (!wrap.contains(e2.target)) { close(); document.removeEventListener('click', handler); } };
            document.addEventListener('click', handler);
        } else { close(); }
    };
    wrap.querySelectorAll('.cs-font-ctx-opt').forEach(opt => {
        opt.onclick = async e => {
            e.stopPropagation();
            const font = opt.dataset.v;
            label.textContent = font;
            trigger.style.fontFamily = `'${font}',sans-serif`;
            close();
            await csApplyFont(font);
            _csRenderContextProps(_csCanvas.getActiveObject());
        };
    });
}
...
window.csOpenCard = function(cardId) {
    csHideOpenModal();
    window.location.href = `/card-creator?card=${encodeURIComponent(cardId)}`;
};
...
function _csShowContextMenu(x, y) {
    ...
    document.getElementById('cs-cm-replace-btn')?.classList.toggle('hidden', !isImage);
    document.getElementById('cs-cm-replace-sep')?.classList.toggle('hidden', !isImage);

    menu.classList.remove('hidden');
}

window.csReplaceImage = function() {
    if (!_csCanvas) return;
    const obj = _csCanvas.getActiveObject();
    if (!obj || obj.type !== 'image') return;
    ...
    input.onchange = async () => {
        ...
        if (!file) return;
        showToast('Uploading replacement...', 'loading');
        try {
            const fd = new FormData();
            ...
            const { url } = await res.json();
            const htmlImg = await _csLoadSafeImage(url);
            if (!htmlImg) throw new Error('Could not load image');
            obj.setElement(htmlImg);
            obj.data = { ...(obj.data || {}), originalUrl: url };
            _csCanvas.renderAll();
            _csPushHistory();
...
let _csStudioLayouts = [];

async function _csLoadStudioLayouts() {
    try {
        const res = await fetch(`${BACKEND_URL}/api/creator/studio-layouts`, { credentials: 'include' });
        if (!res.ok) return;
        _csStudioLayouts = await res.json();
        _csRenderLayoutGrid();
    } catch {}
}

function _csRenderLayoutGrid() {
    const grid = document.getElementById('cs-layout-grid');
    if (!grid) return;
    if (!_csStudioLayouts.length) {
        grid.innerHTML = '<div style="font-size:10px;color:var(--muted);text-align:center;padding:12px 0;">No layouts saved yet</div>';
        return;
    }
    grid.innerHTML = _csStudioLayouts.map(l => `
        <div style="display:flex;align-items:center;gap:6px;padding:5px 6px;border-radius:8px;border:1px solid var(--bdr);background:var(--bg);">
            <span style="flex:1;font-size:11px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHTML(l.name)}</span>
            <button class="cs-vb" title="Load" onclick="csApplyStudioLayout('${escapeHTML(l.id)}')"><i class="bx bx-download"></i></button>
            <button class="cs-vb" title="Delete" style="color:#ef4444;" onclick="csDeleteStudioLayout('${escapeHTML(l.id)}')"><i class="bx bx-trash"></i></button>
        </div>
    `).join('');
}

window.csSaveStudioLayout = async function() {
    if (!_csCanvas) return;
    const name = prompt('Layout name:');
    if (!name?.trim()) return;

    const canvasJson = _csExportLayerJson();
    showToast('Saving layout...', 'loading');
    try {
        await _csRefreshCsrf();
        const res = await fetch(`${BACKEND_URL}/api/creator/studio-layouts`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': _csCsrfToken },
        ...
        await _csLoadStudioLayouts();
    } catch (err) {
        showToast(err.message || 'Save failed', 'error');
    }
};

window.csApplyStudioLayout = async function(id) {
    if (!_csCanvas) return;
    if (!confirm('Load this layout? Current canvas will be replaced.')) return;
    showToast('Loading layout...', 'loading');
    try {
        const res = await fetch(`${BACKEND_URL}/api/creator/studio-layouts/${id}`, { credentials: 'include' });
        if (!res.ok) throw new Error('Fetch failed');
        const layout = await res.json();
        if (!layout?.canvas_json) throw new Error('No canvas data');
        const json = typeof layout.canvas_json === 'string' ? JSON.parse(layout.canvas_json) : layout.canvas_json;
        await _csInitFromJson(json);
...
window.csDeleteStudioLayout = async function(id) {
    if (!confirm('Delete this layout?')) return;
    try {
        await _csRefreshCsrf();
        const res = await fetch(`${BACKEND_URL}/api/creator/studio-layouts/${id}`, {
            method: 'DELETE',
            headers: { 'X-CSRF-Token': _csCsrfToken },
            credentials: 'include',
        });
...
function _csDraftIndicator(state) {
    ...
    if (!dot || !lbl) return;
    if (state === 'hidden') { dot.style.display = 'none'; lbl.textContent = 'Card Creator'; return; }
...
// Castle-defined ATK/DEF position presets (positions within 500×700 artboard)
const _CS_STAT_POS_TEMPLATES = [
    {
        id: 'bottom-row',
        label: 'Bottom Row',
        desc: 'Side by side at the bottom',
        atk: { left: 140, top: 648 },
        def: { left: 360, top: 648 },
    },
    {
        id: 'bottom-corners',
        label: 'Bottom Corners',
        desc: 'Pinned to bottom-left and bottom-right',
        atk: { left: 56,  top: 660 },
        def: { left: 444, top: 660 },
    },
    {
        id: 'top-row',
        label: 'Top Row',
        desc: 'Side by side at the top',
        atk: { left: 140, top: 52 },
        def: { left: 360, top: 52 },
    },
    {
        id: 'top-corners',
        label: 'Top Corners',
        desc: 'Pinned to top-left and top-right',
        atk: { left: 56,  top: 52 },
        def: { left: 444, top: 52 },
    },
];
...
function _csActiveStatTemplate() {
    const id = document.getElementById('cs-stat-pos-template')?.value || 'bottom-row';
    return _CS_STAT_POS_TEMPLATES.find(t => t.id === id) || _CS_STAT_POS_TEMPLATES[0];
}

function _csRenderStatPosTiles() {
    const grid = document.getElementById('cs-stat-pos-grid');
    if (!grid) return;
    const W = 500, H = 700, PW = 60, PH = 84, sx = PW / W, sy = PH / H;
    const selectedId = document.getElementById('cs-stat-pos-template')?.value || 'bottom-row';

    grid.innerHTML = _CS_STAT_POS_TEMPLATES.map(tpl => {
        const ax = (tpl.atk.left * sx).toFixed(1), ay = (tpl.atk.top * sy).toFixed(1);
        const dx = (tpl.def.left * sx).toFixed(1), dy = (tpl.def.top * sy).toFixed(1);
        const isActive = tpl.id === selectedId;
        return `
        <button onclick="_csSelectStatPosTpl('${tpl.id}')" style="
            padding:6px;border-radius:10px;cursor:pointer;
            border:1.5px solid ${isActive ? 'var(--a)' : 'var(--bdr)'};
            background:${isActive ? 'oklch(from var(--a) l c h / 0.1)' : 'oklch(0% 0 0 / 0.2)'};
            display:flex;flex-direction:column;align-items:center;gap:4px;
            transition:all 0.15s;
        " title="${tpl.label} — ${tpl.desc}">
            <svg width="${PW}" height="${PH}" viewBox="0 0 ${PW} ${PH}" style="border-radius:4px;overflow:visible;">
                <rect x="1" y="1" width="${PW-2}" height="${PH-2}" rx="3" fill="oklch(12% 0.01 256)" stroke="${isActive ? 'rgba(104,182,255,0.5)' : 'rgba(255,255,255,0.1)'}" stroke-width="1"/>
                <!-- ATK badge -->
                <rect x="${+ax-10}" y="${+ay-7}" width="20" height="14" rx="3" fill="rgba(239,68,68,0.85)"/>
                <text x="${ax}" y="${+ay+4}" text-anchor="middle" font-size="7" font-weight="900" fill="white" font-family="sans-serif">ATK</text>
                <!-- DEF badge -->
                <rect x="${+dx-10}" y="${+dy-7}" width="20" height="14" rx="3" fill="rgba(59,130,246,0.85)"/>
                <text x="${dx}" y="${+dy+4}" text-anchor="middle" font-size="7" font-weight="900" fill="white" font-family="sans-serif">DEF</text>
            </svg>
            <span style="font-size:8px;font-weight:700;color:${isActive ? 'var(--a)' : 'var(--muted)'};text-transform:uppercase;letter-spacing:0.05em;line-height:1.2;text-align:center;">${tpl.label}</span>
        </button>`;
    }).join('');
}

window._csSelectStatPosTpl = function(id) {
    const inp = document.getElementById('cs-stat-pos-template');
    if (inp) inp.value = id;
    _csRenderStatPosTiles();
    csApplyStatPositionTemplate();
};

window.csApplyStatPositionTemplate = function() {
    if (!_csCanvas) return;
    const tpl = _csActiveStatTemplate();
    const atk = _csCanvas.getObjects().find(o => o.data?.layerType === 'stat' && o.data?.statType === 'atk');
    const def = _csCanvas.getObjects().find(o => o.data?.layerType === 'stat' && o.data?.statType === 'def');
    if (atk) atk.set({ left: tpl.atk.left, top: tpl.atk.top, originX: 'center', originY: 'center', ..._CS_STAT_LOCK });
    if (def) def.set({ left: tpl.def.left, top: tpl.def.top, originX: 'center', originY: 'center', ..._CS_STAT_LOCK });
    if (atk || def) { _csCanvas.renderAll(); _csPushHistory(); }
};
...
window.csAddStatComponent = function(type) {
    ...
    const tpl     = _csActiveStatTemplate();
    const pos     = tpl[type]; // {left, top}

    const txt = new fabric.Text(String(statVal), {
        left: pos.left,
        top: pos.top,
        originX: 'center', originY: 'center',
        fontSize: 36,
...
window.csAddNameComponent = function() {
    if (!_csCanvas) return;
    const existing = _csCanvas.getObjects().find(o => o.data?.layerType === 'name');
    if (existing) { showToast('Name slot already on canvas', 'info'); _csCanvas.setActiveObject(existing); _csCanvas.renderAll(); return; }

    const nameVal = document.getElementById('cs-name')?.value || 'Card Name';
    const font    = document.getElementById('cs-name-font')?.value  || 'Space Grotesk';
    const size    = parseInt(document.getElementById('cs-name-size')?.value, 10) || 36;
    const color   = document.getElementById('cs-name-color')?.value || '#ffffff';

    const txt = new fabric.Text(nameVal, {
        left: CS_W / 2,
        top: 60,
        originX: 'center', originY: 'center',
        fontSize: size,
    ...
    txt.data = { layerName: 'Name', layerType: 'name' };

    _csCanvas.add(txt);
    _csCanvas.setActiveObject(txt);
    _csCanvas.renderAll();
    _csPushHistory(); _csRenderLayerList();
    showToast('Name slot added', 'success');
};

window._csUpdateNameOnCanvas = function() {
    if (!_csCanvas) return;
    const obj = _csCanvas.getObjects().find(o => o.data?.layerType === 'name');
    if (!obj) return;
    const nameVal = document.getElementById('cs-name')?.value || '';
    const font    = document.getElementById('cs-name-font')?.value || 'Space Grotesk';
    const size    = parseInt(document.getElementById('cs-name-size')?.value, 10) || 36;
    const color   = document.getElementById('cs-name-color')?.value || '#ffffff';
    obj.set({ text: nameVal, fontFamily: font + ', sans-serif', fontSize: size, fill: color });
    _csCanvas.renderAll();
};

window._csUpdateStatOnCanvas = function(type) {
    if (!_csCanvas) return;
    const cfg = _CS_STAT_CFG[type];
    if (!cfg) return;
    const obj = _csCanvas.getObjects().find(o => o.data?.layerType === 'stat' && o.data?.statType === type);
    if (!obj) return;
    const val = document.getElementById(cfg.key)?.value || '0';
    obj.set({ text: String(_csEvalStat(val)) });
    _csCanvas.renderAll();
};

...
- [x] **Promo limits** — decision: **per-rerun one-time fee**. Built: migration adds `rerun_fee_cents`, `rerun_cooldown_days`, tracking fields; API endpoint POST `/api/creator/promo-codes/{id}/launch` charges fee + enforces cooldown; dashboard UI shows fee/cooldown, has Launch button ([src/index.ts](src/index.ts), [migrations/093_promo_limits.sql](migrations/093_promo_limits.sql), [dashboard.html](public/dashboard.html)).
